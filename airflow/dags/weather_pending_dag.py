...
def get_missing_dates(s3_hook):
    if os.path.exists("/tmp/s3_files/concatenated_accidents.csv"):
        ...
        dates = (
            pd.to_datetime(
                df[["an", "mois", "jour"]].astype(str).agg("-".join, axis=1),
                errors="coerce",
            )
            .dt.strftime("%Y-%m-%d")
            .dropna()
        ...
        logging.info(
            f"Found {len(dates)} dates in /tmp/s3_files/concatenated_accidents.csv"
        )
    ...
    for key in accident_keys:
        if key.endswith(".csv"):
            ...
            try:
                s3_hook.get_key(key=key, bucket_name=S3_BUCKET_NAME).download_file(
                    local_path
                )
                df = pd.read_csv(
                    local_path, sep=None, engine="python", encoding="ISO-8859-1"
                )
    ...
    return sorted(
        [d for d in accident_dates if d >= "2022-01-01" and d <= "2023-12-31"]
    )
...
def fetch_pending_weather_data():
    ...
    weather_keys = s3_hook.list_keys(bucket_name=S3_BUCKET_NAME, prefix=weather_prefix)
    weather_dates = set()
    for key in weather_keys:
        ...
        if m:
            ...
            m2 = re.search(r"weather_history_(\d{4})_(\d{2})_(\d{2})\\.csv$", key)
            if m2:
    ...
    for date_str in missing_dates:
        ...
        weather_rows = []
        commune_args = [
            (row["com"], row.get("lat"), row.get("lon")) for row in commune_rows
        ]
